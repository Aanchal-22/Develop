...
define(['angular',
    ...
    controllers.controller('loginCtrl', ['$rootScope', '$scope', 'loginService', '$http','$state', '$cookies','$cookieStore', function($rootScope, $scope,loginService, $http, $state, $cookies, $cookieStore) {
        ...
        $scope.loginCheck = function() {
            ...
            loginService.getLoginUserData($scope.loginId).success(function(result)
            {
                ...
                $cookieStore.put('userInfo',JSON.stringify($rootScope.loginUserInfo1));
                $cookieStore.put('usrDfltRole',JSON.stringify($rootScope.dfltRole));
                $cookieStore.put('usrDfltPrivilege',JSON.stringify($rootScope.dfltPrivilege));
                $cookieStore.put('userRole',role);
                $cookieStore.put('userPrivilege',privilege);
                //console.log($rootScope.loginUserInfo.pkid);
